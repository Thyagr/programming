...
public class Program
{
  ...
  private static (int, int) SearchSorteMatrix(int[][] matrix, int value)
  {
    ...
    */
    while (r < rows && c >= 0) 
    {
      if (matrix[r][c] > value) c -= 1; // go left
      else if (matrix[r][c] < value) r += 1; // go down
      else {
        pos = (r, c);
        break;
      }
  ...
  private static void MakeRowsColsZero(int[][] matrix)
  {
    ...
    var zeroRows = new HashSet<int>(); // so same value is not added twice
    ...
    for (int r = 0; r < rows; r++)
    {
      for (int c = 0; c < cols; c++)
      {
        if (matrix[r][c] == 0)
        {
          zeroRows.Add(r);
          zeroCols.Add(c);
          break; // we can break from the row to move to next row
        }
      }
    }
